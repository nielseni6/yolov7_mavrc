...
def test(data,
    ...
    opt.hyp = 'data/hyp.drone.tiny.yaml'
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        if samples_processed > n_samples:
            break
        if not (process_img and (samples_processed <= n_samples)):
            if skip:
            ...
            skip = False
        img_num += 1
        if process_img and (samples_processed <= n_samples):
        ###################################### def generate_attr(): ######################################
            samples_processed += 1
            print("Processing Sample ", samples_processed, "...")
            # print("Inference")
            # Inference
                    ...
                    for cam_num, cam in enumerate(opt.cam_list):
                        ...
                        if cam != None:
                            ...
                            if cam == 'eigen':# or cam == 'eigengrad':# or cam == 'score':
                                ...
                                targets_ = [ClassifierOutputTarget(1)]
                                ...
                                targets_ = [SemanticSegmentationTarget(0, mask_float)]
                            elif cam == 'grad':
                                output = model_seg(img) # model_seg
                                normalized_masks = torch.nn.functional.softmax(output[0], dim=1).cpu()
                                mask = normalized_masks[0, :, :, :].argmax(axis=0).detach().cpu().numpy()
                            ...
                            if cam == 'eigen':
                                ...
                                grayscale_cam = cam_(input_tensor=img,targets=targets_,eigen_smooth=True,aug_smooth=False)
                            
                            if cam == 'grad':
                                cam_ = GradCAM(model_seg, target_layers, use_cuda=False) # model_seg
                                # grayscale_cam = cam_(input_tensor=img.requires_grad_(True),targets=targets_,eigen_smooth=False,aug_smooth=False)
                                # print('pre-cam')
                            ...
                            if cam == "eigengrad": 
                                ...
                                grayscale_cam = cam_(input_tensor=img.requires_grad_(True), targets=targets_,
                            ...
                            if cam == "random":
                                # cam_ = GradCAM(model_seg, target_layers, use_cuda=False) # model_seg
                                # grayscale_cam = cam_(input_tensor=img.requires_grad_(True),
                                #                     targets=targets_)#[0, :] # Placeholder CAM to be replaced with random
