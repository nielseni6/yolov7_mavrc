...
import Perturb_Functions as pf
...
def imshow(img, fig_name="imshowfig", transpose = True):
    ...
    if transpose:
        ...
        npimg_tp=npimg
    plt.imshow(npimg_tp)
    plt.savefig(str("figs/"+fig_name+".png"))
                ...
                if len(det) or not opt.only_detect_true:
                    ...
                    for cam_num, cam in enumerate(opt.cam_list):
                        ...
                        if cam != None:
                            ...
                            print(cam)
                            # imshow(normalize_image(torch.tensor(grayscale_cam)))
                            ...
                            if grayscale:
                                ...
                                rand_attr[0][0], rand_attr[0][1], 
                                rand_attr[0][2] = gray_cam_tensor[0], 
                                gray_cam_tensor[0], gray_cam_tensor[0]

                                attr_multi_channel = rand_attr


                            increment = 1
                            t_ = [round(float(p * increment),2) for p in range(10)]
                            epsilon = 0.25
                            add_or_subtract_attr = False
                            normalize = True
                            for ite in range(10):
                                t = t_[ite]
                                # conver grayscale_cam to three channel
                                formatted_img = pf.attack_data_format(img, attr_multi_channel, 
                                                                      n_steps = int(t), epsilon = epsilon, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--source-labels', type=str, default='inference/labels', help='source for labels')
    ...
    opt.source_labels = '/data/Koutsoubn8/ijcnn_v7data/Real_world_test/labels'
